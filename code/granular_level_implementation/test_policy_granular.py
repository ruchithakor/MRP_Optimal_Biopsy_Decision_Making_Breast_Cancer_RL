...
def perform_action(state, action, age):
    ...
    nan_mask = np.isnan(probabilities) # Check for NaN values
...
def test_policy(policy, num_episodes = 1000, max_prob_state_1 = 0.2, max_prob_state_2 = 0.3):
    ...
    for _ in range(num_episodes):
        ...
        state = round((1 - initialize_occupancy_distribution(max_prob_state_1, max_prob_state_2)[0]) * 100, 1) # get state
...
print(f"Backward Induction Policy granular - Mean Reward: {round(mean_reward_bi, 4)}, Std Reward: {round(std_reward_bi, 4)}")

# Test the Q-learning policy
mean_reward_ql, std_reward_ql = test_policy(policy_q_learning)
print(f"Q-learning Policy granular - Mean Reward: {round(mean_reward_ql, 4)}, Std Reward: {round(std_reward_ql, 4)}")
